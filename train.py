        ...
        for epoch in range(1,args.n_epochs+1):
            ...
            if (epoch) % args.n_epochs == 0:
                with ema.average_parameters():
                    ...
                    f = open(os.path.join(result_save_path, class_name + ".txt"), 'w')
                    f.write(f'image-level auroc : {round(img_auroc,3)}\n')
                    f.write(f'pixel-level auroc : {round(pixel_auroc,3)}\n')
                    f.close()
                    print('Final result -> Category : {}, img_auroc : {}, pixel_auroc : {}'.format(class_name, round(img_auroc,3), round(pixel_auroc,3)))
                    print('Save final model:', os.path.join(model_save_path, class_name + ".pth"))
                    print('Save result file:', os.path.join(result_save_path, class_name + ".txt"))
